...
describe('FlowTest', () => {
  it('perform crud operations via respective endpoints', async () => {
    ...
    const dir = await request(server.app)
      .post('/api/v1/file-manager/directory')
    ...
    res = await request(server.app).post('/api/v1/flowtest').send({ name: 'test', path: dir.text, flowData });
    ...
    res = await request(server.app).put('/api/v1/flowtest').send({ path: flowTestPath, flowData });
